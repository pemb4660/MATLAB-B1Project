function [ T_cw ] = FillImage(T_ow,KMatrix,GridWidth,...
    CameraHeight, CameraWidth)
% FillImage Takes in a Calibration grid definition and positions the
% camera so the the image of the grid fills the camera image.

% T_ow is the Calibration Grid frame
% KMatrix is the intrinic camera model
...
while InsideImage == 1
    ...
    % PositionCamera makes the camera 'almost' point
    % at the object, but introduces randomness so we have
    % a range of angles betweeen the grid's normal and the
    % camera z-axis.
    ...
    % Move the camera nearer to the if any of the corners are
    % inside the image. The next time around we have a better chance
    % of achieving out objective.
    if InsideImage == 1
