...
%    b) If the error went down accept the new parameters and recompute mu using the equation from the notes.
% 9. Go to step 3.
%
% A note on the Jacobian.
% The Jacobian is expensive to compute, thus I keep the same Jacobian
% until it becomes a poor predictor of the change in error - I then
% recompute.
%
...
for nHomog = 1:nImages
    ...
    % The translation part of perspectivity in the camera fram
    ...
    % Find the axis of rotationy finding the real eigenvector
    [V,D] = eig(RotMat);
    D = diag(D);
...
% 2. Compute the initial error vector and the Jacobians and the inner
% product
for j = 1:nImages
...
% The initial value of the exponential growth factor mu
...
while Searching == 1
    ...
    if Iterations > MaxIterations
        error('Number of iterations is too high')
    end
